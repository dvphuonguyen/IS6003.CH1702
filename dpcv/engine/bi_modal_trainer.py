...
class BiModalTrainer(object):
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        loss_list = []
        acc_avg_list = []
        for a, data in enumerate(data_loader):
            ...
            self.tb_writer.add_scalar("loss", loss.item(), a)
            loss.backward()
            optimizer.step()
            ...
            if a % self.cfg.LOG_INTERVAL == self.cfg.LOG_INTERVAL - 1:
                remain_iter = epo_iter_num - a
...
class TPNTrainer(BiModalTrainer):
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for c, data in enumerate(data_loader):
            ...
            if c % self.cfg.LOG_INTERVAL == self.cfg.LOG_INTERVAL - 1:
                self.logger.info(
    ...
    def test(self, data_loader, model):
        ...
        with torch.no_grad():
            ...
            dataset_output = torch.cat(output_list, dim=0).numpy()
            dataset_label = torch.cat(label_list, dim=0).numpy()
            # Add print predicted value and label
            # for i in range(len(dataset_output)):
            #     self.logger.info(
            #         "Test: {inputs} - Predicted Value:  {dataset_output[i]} - Label: {dataset_label[i]}"
            #     )
...
class PersEmoTrainer(BiModalTrainer):
    ...
    def train(self, data_loader, model, loss_f, optimizer, epoch_idx):
        ...
        for e, data in enumerate(data_loader):
            ...
            acc_avg_list.append(acc_avg)
            # print loss info for an interval
            if e % self.cfg.LOG_INTERVAL == self.cfg.LOG_INTERVAL - 1:
                self.logger.info(
                    "Train: Epoch[{:0>3}/{:0>3}] Iteration[{:0>3}/{:0>3}] Loss: {:.4f} Acc:{:.4f}".format(
                        epoch_idx + 1, self.cfg.MAX_EPOCH,
                        e + 1, len(data_loader),
    ...
    def valid(self, data_loader, model, loss_f, epoch_idx):
        ...
        with torch.no_grad():
            ...
            acc_batch_list = []
            ocean_acc_epoch = []
            for f, data in enumerate(data_loader):
                inputs, p_labels, e_labels = self.data_fmt(data)
                p_score, p_co, e_score, e_co, x_ep = model(*inputs)
                loss = loss_f(p_score, p_labels, e_score, e_labels, p_co, e_co, x_ep)
                loss_batch_list.append(loss.item())
