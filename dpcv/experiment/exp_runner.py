...
class ExpRunner:
    ...
    def test(self, weight=None):
        ...
        if not self.cfg.TEST.FULL_TEST:
            ...
            path_dir = os.path.join(".", self.cfg.DATA.ROOT, self.cfg.DATA.TEST_IMG_DATA)
            #Add print predicted value and label
            index_of_pred_lab = 0
            for f in os.scandir(path_dir):
                if f.is_dir():
                    self.logger.info(
                        "Test: Input file: {} - Predicted Value:  {} - Label: {}\n".
                        format(
                            f.name,
                            dataset_output[index_of_pred_lab],
                            dataset_label[index_of_pred_lab]
                        )
                    )
                    index_of_pred_lab = index_of_pred_lab + 1
